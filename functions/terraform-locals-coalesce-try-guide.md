...
**The key thing:** a `variable` block isn't just storage — it's a *door* that lets someone outside your code hand in a different value when they run Terraform. A `locals` block has no such door. Let me show both, side by side.

## How someone actually overrides `var.project_name`

Using your exact code, here are four real ways someone running this Terraform project could change `project_name` without touching your `.tf` file at all:

**1. Command line flag**
```bash
terraform apply -var="project_name=Customer Portal"
```

**2. A `.tfvars` file** (e.g. `prod.tfvars`)
```hcl
...
**3. An environment variable** (notice the `TF_VAR_` prefix)
```bash
export TF_VAR_project_name="Customer Portal"
terraform apply
```

**4. A file Terraform auto-loads** — if a file literally named `terraform.tfvars` sits in the same folder, Terraform reads it automatically, no flag needed.

In every one of these cases, **you never edited `main.tf`**. The `default = "Payments App"` in your `variable` block only kicks in if *none* of the above are used — it's a fallback, not a hard-coded value.

## What happens to `local.clean_name` when that override happens

This is the part that makes it click. Look again:
...
`local.clean_name` is a **formula**, not a stored value. Every time you run `terraform plan`/`apply`, Terraform re-runs that formula from scratch using whatever `var.project_name` currently is.

| `var.project_name` came in as... | `local.clean_name` becomes... |
|---|---|
| `"Payments App"` (the default, no override) | `"payments-app"` |
| `"Customer Portal"` (overridden via `-var`) | `"customer-portal"` |
| `"My Big Project"` (overridden via `.tfvars`) | `"my-big-project"` |

So when I said "the caller can't touch this directly" — I meant exactly that: **literally**. There is no `-var="clean_name=..."` flag. There's no `clean_name.tfvars`. There's no `TF_VAR_clean_name`. Terraform has zero mechanism for an outside user to inject a value straight into a `locals` block. The *only* way `local.clean_name` ever changes is indirectly — by someone overriding `var.project_name`, which then flows through your `lower(replace(...))` formula automatically.

That's the real distinction:
- **Variable** = a labeled input slot with a door other people can open.
- **Local** = a calculation that happens entirely inside your code, with no door — it just reacts automatically to whatever the variables feed it.
